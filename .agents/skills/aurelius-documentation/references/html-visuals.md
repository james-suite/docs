...
- State diagrams may use `layout.canvas`, `layout.states`, and `layout.transitions` when their flow needs authored geometry. A state declares `x`, `y`, and optional `width` or `height`; a transition matches `from`, `to`, and optionally `label`, then accepts `fromSide`, `toSide`, `waypoints`, and `labelPlacement`. Use this to route cycles and terminal paths through open canvas rather than across a state.
